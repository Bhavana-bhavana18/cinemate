...
export const Header = () => {
  ...
  const activeClass = "text-base block py-2 px-3 text-blue bg-brand rounded md:bg-transparent md:text-fg-brand md:p-0 md:dark:text-fg-brand md:dark:bg-transparent"
...
<nav className="border-b-2 bg-neutral-primary fixed w-full z-20 top-0 inset-s-0 border-default dark:border-2 dark:border-gray-900 mb-5">
  ...
  <div id='mobile-nav' className="flex items-center md:order-2">
    ...
    <div className="relative hidden md:block">
      ...
      <form onSubmit={handleSubmit} className="flex items-center">
        <input type="text" id="input-group-1" name='search' className="block w-full ps-9 pe-3 py-2.5 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand px-2.5 shadow-xs placeholder:text-body" placeholder="Search.." autoComplete='off'/>
      </form>
    </div>
    <button data-collapse-toggle="navbar-search" type="button" className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-body rounded-base md:hidden hover:bg-neutral-secondary-soft hover:text-heading focus:outline-none focus:ring-2 focus:ring-neutral-tertiary" onClick ={() => setHidden(!hidden)}  aria-controls="navbar-search" aria-expanded="false">
